...
class _ItemDetailState extends State<ItemDetail> {
  ...
  Future<void> getRecord() async {
    String uri =
        "${globals.uriname}itemgroup_read.php?IM_ItemId=${widget.itemid}&clientcode=${globals.clientcode}&cmp=${globals.cmpcode}";
    try {
        ...
        if (jsonResponse is List) {
          setState(() {
            ...
            for (var item in userData) {
              ...
              _controllers
                  .add(TextEditingController(text: item["IM_Prefix"] ?? "N/A"));
            }
          });
        } else {
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            TextFormField(
                              controller: TextEditingController(
                                  text: item["IM_ItemCode"] ?? "N/A"),
                              decoration: const InputDecoration(
                                border: OutlineInputBorder(),
                                labelText: 'ItemGroup Code',
                              ),
                              readOnly: true,
                            ),
                            const SizedBox(height: 24),
                            ...
                            const SizedBox(
                                height:
                                    24), // Adds vertical space before images
                            TextFormField(
                              controller: TextEditingController(
                                  text: item["IM_Prefix"] ?? "N/A"),
                              decoration: const InputDecoration(
                                border: OutlineInputBorder(),
                                labelText: 'Prefix',
                              ),
