...
class ChallanNew extends StatefulWidget {
  final String omid;
  final String omdate;
 ...
 ChallanNew({
  ...
  @override
  _ChallanNewState createState() => _ChallanNewState();
}

class _ChallanNewState extends State<ChallanNew> {
  late String uri;
  ...
  Future<void> _fetchOrderId() async {
    String uri =
        "${globals.uriname}challan_get_challan_no.php?clientcode=${globals.clientcode}&cmp=${globals.cmpcode}";
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['status'] == 'success') {
          ...
          widget.ChallanIdController.text = orderNo;
          setState(() {});
        } else {
...
Future<void> fetchOrderDetails() async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      print('Full response data: $data'); // Print entire response for debugging

      if (data.isNotEmpty) {
        setState(() {
          // Mapping customers, agents, and transports
          customers = data
              .map((order) => {
                    'AM_AccId': order['om_custid'],
                    'AM_AccName': order['CustName'],
                  })
              .toList();
          selectedCustomer = customers.isNotEmpty ? customers[0]['AM_AccId'] : null;
          ...
          agents = data
              ...
              .toList();
          selectedAgent = agents.isNotEmpty ? agents[0]['ID'] : null;

          transports = data
              .map((order) => {
                    'TransportID': order['om_transportid'],
                    'TransportName': order['om_transport'],
                  })
              .toList();
          selectedTransport =
              transports.isNotEmpty ? transports[0]['TransportID'] : 'null';
          ...
          // Calculate totals
      totalQuantity = data.fold<int>(
  0, (int sum, order) {
    final quantity = int.tryParse(order['ot_invqty']?.split('.')[0] ?? '0') ?? 0;
    return sum + quantity;
  }
);


totalItems = data.length; // Total number of rows (items)
totalAmount = data.fold(
    0.0, (sum, order) => sum + double.tryParse(order['ot_subtotal'] ?? '0.00')!);

          // Process orders
          orders = data.map((order) {
  return {
    'itemId': order['ot_itemid']?.toString() ?? '',
    'quantity': order['ot_invqty']?.toString() ?? '',
    'rate': order['ot_rate']?.toString() ?? '',
    // 'totalAmount': order['ot_subtotal']?.toString() ?? '',  // Ensure string conversion
    'itemName': order['IM_ItemName']?.toString() ?? '',      // Safely handle item name
  };
}).toList();

          print('Orders after mapping: $orders'); // Debug: Print orders
...
Future<void> saveChallanData() async {
  final url = Uri.parse('${globals.uriname}challan.php');
  ...
  final payload = {
    ...
    "orders_data": [
      {
        "om_invno": int.tryParse(widget.ChallanIdController.text) ?? 0,
        "om_orderno":int.tryParse(widget.IdController.text) ?? 0,
        "om_orderdate":widget.DateController.text.isNotEmpty
            ? widget.DateController.text
            : DateTime.now().toIso8601String(),
        "om_date": widget.DateController.text.isNotEmpty
            ? widget.DateController.text
            : DateTime.now().toIso8601String(),
        "om_custid": selectedCustomer,
        "om_agentid": selectedAgent,
        "om_transportid": selectedTransport,
        "om_transport": transports.firstWhere((transport) =>
            transport['TransportID'] == selectedTransport)['TransportName'],
        "om_qty": totalQuantity,
        "om_billamt": totalAmount,
        "om_noofitems": totalItems, // Add the total number of items
        // "om_id": widget.omid, // Ensure you include the om_id for the update
        "order_details": orders.asMap().entries.map((entry) {
  int index = entry.key;
  var item = entry.value;
  return {
    "ot_srno": index + 1,
    "ot_itemid": item['itemId'],
    "ot_invqty": double.tryParse(item['quantity'] ?? '0') ?? 0,
    "ot_rate": double.tryParse(item['rate'] ?? '0') ?? 0.0,
    "ot_subtotal": double.tryParse(item['totalAmount'] ?? '0') ?? 0.0,
  };
}).toList(),
  ...
  try {
    // Send the HTTP POST request to update the order data
    ...
    if (response.statusCode == 200) {
      ...
      if (responseData['status'] == 'success') {
        print('Challan data successfully updated');
        ...
        // Navigate back to the previous page with a result indicating success
        Navigator.pop(context, true); // true indicates that data was updated
      } else {
        // Server-side error, print the message from the response
        print('Server Error: ${responseData['message']}');
        
        // Notify the user about the error
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Server Error: ${responseData['message']}'),
            duration: Duration(seconds: 2),
          ),
        );
      }
    } else {
      // HTTP error occurred, log the status code
      print('HTTP Error: ${response.statusCode}');
...
const SizedBox(height:10),
                ...
                final updatedOrders = await Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (context) => ItemListPage(
                      ...
                      orders: List.from(
                          orders), // Pass a copy of the current orders
  ...
  onChanged: (value) {
    setState(() {
      ...
      if (index != -1) {
        ...
        orders[index]["quantity"] = int.tryParse(value) ?? 0;
 ...
 calculateTotals();
      ...
      calculateTotals();
                                          ...
                                          const SizedBox(width: 9), // Space between columns
          ...
          Text(
            // Multiply Qty and Rate after parsing to int or double
            '${(double.tryParse(item["quantity"].toString()) ?? 0) * (double.tryParse(item["rate"].toString()) ?? 0.0)}',
                                                ...
                                                Row(
                                                  ...
                                                  children: [
                                                    ...
                                                    IconButton(
                                                      ...
                                                      onPressed: () {
                                                        showEditDialog(index); // Open the edit dialog
                                                      },
                                                    ),
                                                    IconButton(
            ...
            Row(
              children: [
                Expanded(
                  child: Text(
  'Total Qty: ${totalQuantity.toStringAsFixed(0)}',
  ...
  children: [
    Expanded(
     ...
     onPressed: () async {
  print('Save challan button pressed');  // Print when the button is pressed

  ScaffoldMessenger.of(context).showSnackBar(
    const SnackBar(
      content: Text('Saving challna...'),
      duration: Duration(seconds: 2),
    ),
  );

  try {
    print('Calling updateOrderData function');  // Print before calling the updateOrderData function
    await saveChallanData();  // Make sure this call is awaited properly
    print('Challan data successfully updated');  // Print if the order is successfully updated
    ...
    print('Error encountered: $error');  // Print the error message
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
