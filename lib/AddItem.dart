...
Future<void> fetchItemData(String barcode) async {
  ...
  if (barcode == _lastScannedBarcode && (currentTime - _lastScannedTime) < 1000) {
...
void showSnackBar(String message) {
  ...
  void _fetchItemNames({String search = ''}) async {
    ...
    final uri = "${uriname}item_list_2.php?clientcode=$clientcode&cmp=$cmpcode&search=${Uri.encodeComponent(search)}";
    print(uri);
    setState(() {
      isLoading = true;
    });
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          _filteredItems = search.isEmpty
              ? _items
              : _items.where((item) {
                  final itemName = item['IM_ItemName']?.toString().toLowerCase() ?? '';
    ...
    children: [
      ...
      ...widget.savedItems.map((savedItem) {
        ...
        final selectedItem = _items.firstWhere(
          ...
          orElse: () => {}, // Provide an empty map if item is not found
        );
              ...
              children: [
                ...
                Positioned(
                  ...
                  child: IconButton(
                    ...
                    onPressed: () {
                      setState(() {
                        widget.savedItems.removeWhere((savedItem) => savedItem['itemId'] == itemId);
                      });
                    },
                  ),
                ),
            ...
            child: Stack(
              children: [
                ...
                Positioned(
                  ...
                  right: 0,
                  child: IconButton(
                    icon: const Icon(Icons.close, size: 19,color:Colors.black),
                    padding: const EdgeInsets.all(0),
                    constraints: const BoxConstraints(),
                    onPressed: () {
                      setState(() {
                        _selectedItemIds.remove(itemId); // Remove from selected items
                      });
                    },
                  ),
                ),
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Row(
                            ...
                            children: [
                              ...
                              item2 != null ? _buildItemCard(item2) : Container(),
                            ],
                          );
                        },
                      ),
