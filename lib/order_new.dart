...
//  var item;
    int x1 = 0;
    if (custTypeController.text == 'W') {
      x1 = (double.tryParse(item["rate"].toString())
              ?.toStringAsFixed(0)
              .length ??
          0);
    } else if (custTypeController.text == 'R') {
      x1 = (double.tryParse(item["rate2"].toString())
              ?.toStringAsFixed(0)
              .length ??
          0);
    }
  ...
  void calculateTotals() {
    ...
    for (var order in orders) {
      ...
      amount += rate * qty;
      quantity += qty
          .toInt(); // Convert quantity to int if needed for total quantity calculation
      ...
      print(
          "Interim Amount: $amount, Interim Quantity: $quantity, Interim Items: $items");
    }
    ...
    print(
        "Final Totals -> Amount: $totalAmount, Quantity: $totalQuantity, Items: $totalItems");
  }
  ...
  Future<void> saveOrderData() async {
    ...
    // Assuming global.userid holds the user ID
    String userId = globals.userid; // Use the global variable for user_id
    print(userId);
      ...
      "orders_data": [
        {
          ...
          "om_custrefno":
              (int.tryParse(custrefController.text) ?? 0).toString(),
            ...
            return {
              ...
              "ot_invqty":
                  (int.tryParse(item['quantity']?.toString() ?? '0') ?? 0)
                      .toString(),
              "ot_rate":
                  (double.tryParse(item['rate']?.toString() ?? '0.0') ?? 0.0)
                      .toString(),
              "ot_subtotal":
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 'success') {
          ...
          final String orderId = widget.IdController.text;
        } else {
  ...
  Future<void> fetchCustomerDetails(
      int custId, String clientCode, String username) async {
      ...
      if (response.statusCode == 200) {
        ...
        if (data.isNotEmpty && data[0]['Status'] == 'True') {
          ...
          setState(() {
            ...
            final String? fetchedTransportId =
                data[0]['CustTransportId']?.toString();
            ...
            if (fetchedTransportId != null && fetchedTransportId != '0') {
              ...
              if (!transports
                  .any((transport) => transport['ID'] == fetchedTransportId)) {
  ...
  void openCustomerDropdown(BuildContext context) {
    ...
    showModalBottomSheet(
      ...
      backgroundColor: Colors.transparent, // Remove bottom sheet shadow
      builder: (context) {
        return Container(
          ...
          decoration: const BoxDecoration(
            ...
            borderRadius: BorderRadius.vertical(
              top: Radius.circular(0), // No rounded corners
            ),
          ),
          child: Scaffold(
            appBar: PreferredSize(
              preferredSize:
                  const Size.fromHeight(80), // 👈 increase AppBar height
              child: AppBar(
              ...
              builder: (context, setModalState) {
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      title: Text(customer['AM_AccName'] ?? 'Unknown Customer'),
                      onTap: () async {
                        setState(() {
                          selectedCustomer =
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          customers = data
              ...
              .toList();
          selectedCustomer =
              customers.isNotEmpty ? customers[0]['AM_AccId'] : '';
          ...
          transports = data
              ...
              .toList();
          selectedTransport =
              transports.isNotEmpty ? transports[0]['TransportID'] : '';
          ...
          int otTotalQty = orders.isNotEmpty
              ? orders.fold(
                  ...
                  (sum, order) =>
                      sum +
                      (double.tryParse(order['quantity'].toString())?.toInt() ??
                          0))
              : 0;
...
// Helper function to show a SnackBar
      ...
      "orders_data": [
        {
          ...
          "om_date": widget.DateController.text.isNotEmpty
              ? widget.DateController.text
              : DateTime.now().toIso8601String(),
          "om_custrefno": (int.tryParse(custrefController.text) ?? 0)
              .toString(), // Use custrefController here
          "om_custrefdate": widget.DateController.text.isNotEmpty
              ? widget.DateController.text
              : DateTime.now().toIso8601String(),
          "om_custid": selectedCustomer?.toString() ?? '',
                              ...
                              builder: (context) {
                                try {
                                  final agentName = agents.firstWhere(
                                        ...
                                        orElse: () {
                                          print(
                                              'No matching agent found for selectedAgent: $selectedAgent');
                                                    ...
                                                    child: TextField(
                                                      ...
                                                      onChanged: (value) {
                                                        setState(() {
                                                          ...
                                                          item["rate"] =
                                                              newRate; // Store as double
                                                          item["rate2"] =
                                                              newRate;
                                                          item["subtotal"] =
                                                              (newRate ?? 0) *
                                                                  quantity; // Correct subtotal calculation
                      ...
                      children: [
                        ...
                        Expanded(
                          child: ElevatedButton(
                            onPressed: (!widget.isApproveDisabled &&
                                    isUpdateEnabled)
                                ? () async {
                                    try {
  ...
  Widget build(BuildContext context) {
    bool isUpdateEnabled = double.tryParse(widget.omQty.toString()) ==
        double.tryParse(widget.omPendingQty.toString());

    return Scaffold(
      appBar: AppBar(
        backgroundColor: Theme.of(context).primaryColor,
        title: widget.flag == "Edit"
