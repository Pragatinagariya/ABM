...
class ItemListNewState extends State<ItemListNew> {
  ...
  void _fetchItemNames({String search = ''}) async {
    ...
    final uri =
        "${globals.uriname}item_list.php?clientcode=${globals.clientcode}&cmp=${globals.cmpcode}&search=${Uri.encodeComponent(search)}";
  ...
  void _applyCustomRangeFilter(String startRange, String endRange) {
    setState(() {
      filteredData = filteredData.where((item) {
        final itemName = item["IM_ItemName"];
        if (itemName != null) {
    ...
    return Column(
      children: [
        ...
        DropdownButton<String>(
          value: groupFilter == ''
              ? '--Select--'
              : groupFilter, // Default to '--Select--'
          hint: Text('Select Group'), // Placeholder text
      ...
      children: [
        ...
        SizedBox(
          ...
          child: RangeSlider(
            ...
            min: 0, // Min value
            max: 1000, // Max value
            divisions: 100, // Adjust for finer granularity
            labels: RangeLabels(
  ...
  void _applyFilters() {
    setState(() {
      ...
      if (groupFilter != '--Select--' && groupFilter.isNotEmpty) {
        filteredData = filteredData.where((item) {
          return item["IM_GroupName"] == groupFilter;
        }).toList();
      }
...
Future<void> _initializeDatabase() async {
  ...
  Future<void> _loadSwitchOptions() async {
    final List<Map<String, dynamic>> options = await _database.query('z_settings', where: 'z_page = ?', whereArgs: ['Item']);
    if (options.isEmpty) {
            ...
            return Column(
              ...
              children: settings.map((option) {
                return SwitchListTile(
                  ...
                  activeColor: globals.themeColor,  // Customize the active color
  ...
  for (var image in _images) {
    try {
      var request = http.MultipartRequest(
        'POST',
        Uri.parse("${globals.uriname}upload_image.php?clientcode=$clientcode&cmp=$cmpcode"),
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 'success') {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text("Failed to upload image: ${responseData['message']}")),
          );
        }
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
              ...
              onPressed: () async {
                if (_selectedItemIds.isNotEmpty) {
                  ...
                  for (var itemId in _selectedItemIds) {
                    ...
                    if (item.isNotEmpty) {
                      final imageUrl =
                          '${globals.uriname}${globals.clientcode}/${globals.xyz}/Images/Items/${item['IM_ItemId']}_1.jpg';
print(imageUrl);
                      try {
                        // Download image
                  ...
                  child: Row(
                    children: _selectedItemIds.map((itemId) {
                      final selectedItem = _items.firstWhere(
                        (item) => item['IM_ItemId'].toString() == itemId,
                        orElse: () =>
                            {}, // Provide an empty map instead of null
                      );
                              ...
                              Column(
                                children: [
                                  ClipRRect(
                                    ...
                                    child: Image.network(
                                      '${globals.uriname}${globals.clientcode}/${globals.xyz}/Images/Items/${selectedItem["IM_ItemId"]}_1.jpg',
                                      width: 60,
                                      height: 60,
                                      fit: BoxFit.cover,
                                      errorBuilder:
                                  ...
                                  SizedBox(
                                    width: 80,
                                    child: Text(
                                      selectedItem['IM_ItemName'] ?? 'Unknown',
                                      style: const TextStyle(
                                          fontSize: 10,
...
// Function to show bottom sheet with layout options
  ...
  Widget _buildSingleColumnCard(Map<String, dynamic> item) {
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            'Code: ${item["IM_ItemCode"] ?? "N/A"}',
                            style: const TextStyle(fontSize: 13),
                          ),
                          if (_isFieldVisible('Display Name'))
                          Text(
                            'Display Name: ${item["IM_ItemAlias"] ?? "N/A"}',
                            style: const TextStyle(fontSize: 13),
                          ),
                          if (_isFieldVisible('Sales Rate'))
                          Text(
                            'Sales Rate: ${item["IM_SRate1"] ?? "0.0"}',
                            style: const TextStyle(fontSize: 13),
                          ),
                          if (_isFieldVisible('Purchase Rate'))
                          Text(
                            'Purchase Rate: ${item["IM_PRate1"] ?? "0.0"}',
                            style: const TextStyle(fontSize: 13),
                          ),
                          if (_isFieldVisible('Unit'))
                          Text(
                            'Unit: ${item["UM_UnitCode"] ?? "N/A"}',
                            style: const TextStyle(fontSize: 13),
                          ),
                          if (_isFieldVisible('HSN Code'))
                          Text(
                            'HSN: ${item["IM_HSNCode"] ?? "0.0"}',
                            style: const TextStyle(fontSize: 13),
                          ),
                          if (_isFieldVisible('Barcode'))
                          Text(
                            'Barcode: ${item["IM_Extra5"] ?? "0.0"}',
                            style: const TextStyle(fontSize: 13),
                          ),
                        ],
                      ),
  ...
  Widget _buildTwoColumnCard(Map<String, dynamic> item) {
    final String itemId = item['IM_ItemId'].toString();
    final bool isSelected = _selectedItemIds.contains(itemId);
              ...
              child: Column(
                ...
                children: [
                  ...
                  Padding(
                          ...
                          child: Text(
                            '(${item["IM_GroupName"] ?? "N/A"})', // Added parentheses
                            style: const TextStyle(
  ...
  Widget _buildThreeColumnCard(Map<String, dynamic> item) {
    final String itemId = item['IM_ItemId'].toString();
    final bool isSelected = _selectedItemIds.contains(itemId);
...
// Sample image function to display item images
 Widget _buildImage(Map<String, dynamic> item) {
  String imageUrl = '${globals.uriname}${globals.clientcode}/${globals.xyz}/Images/Items/${item["IM_ItemId"]}_1.jpg';
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) => PhotoViewPage(
            itemname: item["IM_ItemName"] ?? 'No Name',
            imageUrl: item["IM_ItemId"] != null && item["IM_ItemId"].isNotEmpty
                ? '${globals.uriname}${globals.clientcode}/${globals.xyz}/Images/Items/${item["IM_ItemId"]}_1.jpg'
                : 'assets/images/icons/00000000.jpg',
                // print(imageUrl);
          ),
    ...
    child: Container(
      ...
      child: Image.network(
        ...
        errorBuilder: (context, error, stackTrace) {
          return Image.asset('assets/images/icons/00000000.jpg', fit: BoxFit.contain);
        },
      ),
    ),
  );
...
class PhotoViewPage extends StatelessWidget {
  ...
  const PhotoViewPage({super.key, required this.imageUrl, required this.itemname});
  ...
  return Scaffold(
    ...
    body: Center(
      child: PhotoViewGallery.builder(
        itemCount: 1, // If you have more images, increase this number
