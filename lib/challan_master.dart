...
import 'challan_transaction.dart';
...
class ChallanMaster extends StatefulWidget {
  ...
  @override
  State<ChallanMaster> createState() => ChallanMasterState();
}
class ChallanMasterState extends State<ChallanMaster> {
 List userData = [];
  String nameFilter = '';
  List<String> Names = [];
  String selectedFilter = '';
  String selectedNameRange = 'A-Z';
  ...
  Future<void> getRecord() async {
    String uri =
        "${uriname}challan_master.php?clientcode=$clientcode&cmp=$cmpcode";
...
void _applyFilters() {
  ...
  Future<void> _sharePDF(String imid) async {
    final apiUrl =
        '${uriname}sales_challan_pdf.php?clientcode=$clientcode&cmpcode=$cmpcode&IM_Id=$imid';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data is List) {
          ...
          final file = File('${directory.path}/Sales Challan.pdf');
          await file.writeAsBytes(await pdf.save());

          // Share the PDF file
          await Share.shareFiles([file.path], text: 'Sales Challan PDF');
        } else {
...
Future<void> challanList() async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (data is List) {
        final pdf = pw.Document();
        const int itemsPerPage = 30; // Items per page
                    ...
                    return InkWell(
                      onTap: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => ChallanTransaction(
