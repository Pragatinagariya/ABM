...
class InvoiceMaster extends StatefulWidget {
  ...
  @override
  State<InvoiceMaster> createState() => InvoiceMasterState();
}

class InvoiceMasterState extends State<InvoiceMaster> {
 List userData = [];
  String nameFilter = '';
  List<String> Names = [];
...
String selectedItem = '--Select--';
  ...
  Future<void> getRecord() async {
    String uri =
        "${uriname}invoice_master.php?clientcode=$clientcode&cmp=$cmpcode";
...
void _applyFilters() {
  ...
  Future<void> _sharePDF(String imid) async {
    final apiUrl =
        '${uriname}sales_invoice_pdf.php?clientcode=$clientcode&cmp=$cmpcode&IM_Id=$imid';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data is List) {
          ...
          final file = File('${directory.path}/Sales Invoice.pdf');
          await file.writeAsBytes(await pdf.save());

          // Share the PDF file
          await Share.shareFiles([file.path], text: 'Sales Invoice PDF');
        } else {
...
Future<void> custWise() async {
  final apiUrl =
      '${uriname}sales_invoice_custwise.php?clientcode=$clientcode&cmp=$cmpcode';
...
Future<void> _initializeDatabase() async {
 ...
 Future<void> _loadSwitchOptions() async {
  final List<Map<String, dynamic>> options = 
      await _database.query('z_settings', where: 'z_page = ?', whereArgs: ['Sales Invoice']);

  if (options.isEmpty) {
    // Insert default values for Supplier page if not present
    await _database.insert('z_settings', {
      'z_page': 'Sales Invoice',
      'z_flag': '',
      'z_keyword': 'Transport',
      'z_keyvalue': 1,
      'z_remarks': ''
                    ...
                    return InkWell(
                      onTap: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => InvoiceTransaction(
